...
class LiPlugin {
  ...
  static const MethodChannel _channel = MethodChannel('example.com/liplayer');
  static Future<int> _createPlayer() async {
    int? pid = await _channel.invokeMethod("createPlayer");
    if (pid != null) {
      return Future.value(pid);
    }
    LiLog.e("failed to create native player");
  ...
  static void _onLoad(String type) {
    if (_eventSubs == null) {
      LiLog.i("_onLoad $type");
      _eventSubs = const EventChannel("befovy.com/Li/event")
          .receiveBroadcastStream()
          .listen(LiPlugin._eventListener, onError: LiPlugin._errorListener);
    }
  ...
  static void _eventListener(dynamic event) {
    ...
    LiLog.d("plugin listener: $map");
    switch (map['event']) {
      case 'volume':
        bool sui = map['sui'] ?? false;
        double vol = map['vol'] ?? 0.0;
        LiVolume._instance._onVolCallback(vol, sui);
        break;
      default:
        break;
    }
  ...
  static void _errorListener(Object obj) {
    LiLog.e("plugin errorListerner: $obj");
  }
}
